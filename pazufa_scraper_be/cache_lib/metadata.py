...
class Metadata(BaseModel):
    ...
    @classmethod
    def _coerce_key(cls, v: Any) -> Any:  # noqa: ANN401
        """Coerce raw key values to registered Key sub classes.

        This allows to handle round-trip serialization to JSON using sub classes of Key.
        """
        for sub_key_class in Key.__subclasses__():
            try:
                return sub_key_class(v)

            except ValueError:
                continue

        msg = f"Unknown Key value: {v!r}. Is the Key subclass registered?"
